...
from models.market_model import calc_topdown, calc_som, calc_sam, SCENARIOS
...
with c1:
    st.markdown(f"""<div class="metric-card">
        ...
        <div class="metric-value">${tam_glob/1e9:.2f}B</div>
        <div class="metric-sub">BioScope niche, outsourced</div>
    </div>""", unsafe_allow_html=True)

with c2:
    st.markdown(f"""<div class="metric-card" style="border-left-color:#3498DB">
        <div class="metric-label">US TAM</div>
        <div class="metric-value">${tam_us/1e9:.2f}B</div>
...
summary_data = {
    'Level': ['TAM — Global (BioScope niche)', 'TAM — North America', 'TAM — US (Primary)',
              'SAM — North America', 'SOM — Year 1', 'SOM — Year 2', 'SOM — Year 3'],
...
ratio = som_y3 / rob_y5
st.info(
    f"**Cross-check:** Our independently derived Y3 SOM (${som_y3/1e6:.0f}M) is **{ratio:.1f}×** "
    f"BioScope's stated Y5 ARR target (${rob_y5/1e6:.0f}M). "
    f"This suggests the independent model captures larger potential — use Scenario Analysis to stress-test assumptions."
)
